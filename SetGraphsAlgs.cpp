...
void SetGraphAlgs::processDFS(int k, vector<bool> &used, queue<pair<int, int>> &path)
{
	used[k] = true;
...
queue<pair<int, int>> SetGraphAlgs::DFS(int v)
{
...
void SetGraphAlgs::processBFS(int v, vector<int> &price, queue<pair<int, int>> &path)
{
	price[v] = 1;
	queue<int> q;
	q.push(v);
...
queue<pair<int, int>> SetGraphAlgs::BFS(int start, vector<int> &price)
{
	queue<pair<int, int>> path;
...
vector<int> SetGraphAlgs::Euler()
{
	SetGraph *my_copy = new SetGraphAlgs(*this);
	vector<int> degree(my_copy->CountVertex(), 0);
	for (int i = 0; i < my_copy->CountVertex(); ++i) {
		degree[i] += my_copy->get_graph()[i].size();
	}
	int first = 0;
	int v1 = -1, v2 = -1;
...
void SetGraphAlgs::dfs_bridges(int v, int p, int &timer, vector<bool> &used, vector <int> &tin, vector<int> &fup, vector<pair<pair<int, int>, bool>> &ans) {
	used[v] = true;
	timer++;
	tin[v] = timer;
	fup[v] = timer;
...
int SetGraphAlgs::search_ans(vector<pair<pair<int, int>, bool>> &ans, pair<pair<int, int>, bool> &element) {
	if (find(ans.begin(), ans.end(), element) != ans.end())
		return find(ans.begin(), ans.end(), element) - ans.begin();
	return -1;
}

vector<pair<pair<int, int>, bool>> SetGraphAlgs::find_bridges() {
	int timer = 0;
	vector<bool> used(CountVertex(), false);
